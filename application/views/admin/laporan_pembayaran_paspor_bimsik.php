<html>
	<head>
		<title> Laporan Pembayaran Paspor Haji </title>
	...
	<body>
		...
		?>		
		<center><h4> Laporan Pembayaran Papsor Haji <br> KBIH Al-Hidayah Kota Cirebon <br> Bulan <?php echo $blnnama; echo " "; echo date('Y'); ?></h4></center>
		<table width="100%" border="solid" bgcolor="white" align="center">
			...
			<?php
               $no_urut = 0;
               foreach($laporan_pembayaran_paspor_bimsik as $row)
               {
               	  $no_urut++;
                  ?>
                  <tr>
		...
		<script>
			function print_d(){
				window.open("<?php echo base_url();?>index.php/welcome/print_pembayaran_paspor_bimsik","_blank");
			}
			function export_e(){
				window.open("<?php echo base_url();?>index.php/welcome/export_excel_pembayaran_paspor_bimsik");
			}
		</script>
		<br>
